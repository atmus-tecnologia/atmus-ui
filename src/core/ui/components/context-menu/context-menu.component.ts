...
export class AtmContextMenuTrigger {
  readonly atmContextMenu = input.required<AtmContextMenu>();
  readonly atmContextMenuData = input<unknown>();
  readonly atmContextMenuHeader = input<string>();
  readonly atmContextMenuDisabled = input(false);

  protected onContextMenu(event: MouseEvent): void {
    if (this.atmContextMenuDisabled()) return;
    event.preventDefault();
    event.stopPropagation();
    this.atmContextMenu().open(event, {
      data: this.atmContextMenuData(),
      header: this.atmContextMenuHeader(),
    });
  }
}
